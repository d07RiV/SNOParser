...
#pragma	pack(pop)
  ...
  bool imWritePNG(Image const& image, File& file) {
    ...
    write_chunk('IDAT', csize, &cdata[0], file);
    write_chunk('IEND', 0, NULL, file);
  ...
  bool imReadPNG(Image& image, File& file) {
    ...
    if (ch.type != 'IHDR' || ch.length != sizeof hdr) return false;
    memcpy(&hdr, ch.data, sizeof hdr);
    hdr.width = _byteswap_ulong(hdr.width);
    hdr.height = _byteswap_ulong(hdr.height);
